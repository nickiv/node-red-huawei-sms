      ...
      states: {
        ...
        POLL: {
          ...
          poll_success: function (smsXML) {
            ...
            for (var i = 0; i < messages.length; i++) {
              const msgDate = new Date(messages[i].Date);
              if (this.node.concat) {
                if (messages[i].SmsType == 2) {
                  const msgAge = Date.now() - msgDate.getTime();
                  if (msgAge < 30 * 1000) {
                    this.debug(`Ignoring message ${messages[i].Index} because it's too new (${msgAge}ms)`);
                    continue;
                  }
                }
              }
